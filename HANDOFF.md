...
**Unreleased on main after v1.7.1 (2026-09-23): the build no longer embeds the build machine's folders.** Every shipped DLL through 1.7.1 carried the build machine's absolute PDB path in its PE debug directory (`C:\Users\<name>\…\obj\Release\TheRavensCall.pdb` since 1.3.0, a `/home/<user>/…` path before that). `TheRavensCall.csproj` now sets `DeterministicSourcePaths` and always names the project folder as a `SourceRoot` (in a git checkout git adds the same root and the two merge), so the DLL carries `/_/obj/Release/TheRavensCall.pdb` and neither the DLL nor the PDB names a local path. Without a usable git (a copy without `.git` such as GitHub's source zip, an empty or dangling `.git`, git queries switched off, no implicit Source Link) the DLL carries `/_1/obj/Release/TheRavensCall.pdb` instead (the NuGet root takes `/_/`), still path-free, with no Source Link table; all five of those cases build the same bytes. Two clean builds of the same commit are byte-identical in any git clone of the repository, wherever it sits, on Windows with Git's default CRLF checkout and the same .NET SDK (proven: two folders at different paths, and two reviewer clones, one path with a space). A clone with a different origin URL differs, because the PDB's Source Link URL carries it; a Linux clone or an as-is (LF) checkout differs too, because the repo has no `.gitattributes`. The IL is unchanged (identical to 1.7.1 apart from static-data offsets, which move because the debug directory shrank). **At the next cut:** in the changelog's Changed section, say the DLL no longer carries "an absolute build path that included the build machine's user name" (quote no path; DLLs already published are unchanged); word the reproducibility line as "in any git clone of the repository (Windows, Git's default line endings, the same .NET SDK)", no longer "in the same checkout"; and name the commit the DLL was built from ("a clean build of commit abcdef1"), because the md5 follows the commit: the PDB's Source Link URL carries the sha, so a rebuild of the tag commit itself differs in the 72 build-identity bytes. (Option, not taken: `<EnableSourceLink>false</EnableSourceLink>` would make the md5 follow only the source, since the PDB is never shipped.) Porting this to a mod whose csproj sits in a subfolder: add the repo root as the `SourceRoot` (e.g. from a `Directory.Build.props` there), not the project folder, or the project becomes a second root outside Source Link.

**Status: v1.7.1 "The Raven Missed Something" CUT 2026-09-23 (PR #22 MERGED = main `7912e3e`; shipped DLL md5 `380cf11e4aa79948ad1350ad6d08c3f4`, 279,552 bytes, IL-identical to the build below; LIVE on Storm10 with build 4b40afe3: logout-to-menu leave and shutdown leave both write no count, rows `online_count` 0; dead-connection leave waits the 90 s crossplay timeout; two-player shutdown not run) — the leave line's `(N online)` no longer counts the player who left: `Plugin.OnlineCount()` counts `GetConnectedPlayers()` with an open session (`Patch_PlayerJoin.HasOpenSession`) for both the suffix and the Chronicle/feed `online_count`. Version 1.7.1; DLL rebuilt at the cut.**
