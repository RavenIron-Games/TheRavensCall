...
| `theravenscall.html` (4468 lines) | The dashboard, renamed from `stevecompanion.html`. Crew/relay tab fully removed (dead feature in the source — referenced `/api/crew` endpoints that don't exist anywhere in the mod). Branding strings updated (title, wizard copy, DLL filename in troubleshooting text, firewall executable name — was `valheim.exe`, now `valheim_server.exe`, since this runs on the server not a gaming PC). |
...
- **Highest severity:** the dashboard's JS was never updated to match the new field names —
  ...
  bottom of `Companion.AppendRegistryFields`.
...
- The dashboard's `tamed` creature list (`Companion.cs`, inside `BuildStateJson`) is built from
