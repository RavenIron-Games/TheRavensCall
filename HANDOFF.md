...
**Unreleased on main after v1.7.1 (2026-09-23): the build no longer embeds the build machine's folders.** Every shipped DLL through 1.7.1 carried the absolute PDB path (`C:\Users\<name>\source\repos\TheRavensCall\obj\Release\TheRavensCall.pdb`) in its PE debug directory. `TheRavensCall.csproj` now sets `DeterministicSourcePaths` (plus a `SourceRoot` of the project folder when there is no `.git`), so the DLL carries `/_/obj/Release/TheRavensCall.pdb` and neither the DLL nor the PDB names a local path. Two builds of the same commit are byte-identical in any checkout folder (proven: this checkout twice + a worktree at another path; a copy without `.git` is clean too). The IL is unchanged (identical to 1.7.1 apart from static-data offsets, which move because the debug directory shrank). **At the next cut:** say so in the changelog's Changed section, and word the reproducibility line as "in any checkout", no longer "in the same checkout".
