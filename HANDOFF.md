...
**Status: v1.7.1 IN PROGRESS 2026-09-23 (branch `fix/leave-online-count-1.7.1`, not yet merged) — the leave line's `(N online)` no longer counts the player who left: `Plugin.OnlineCount()` counts `GetConnectedPlayers()` with an open session (`Patch_PlayerJoin.HasOpenSession`) for both the suffix and the Chronicle/feed `online_count`. Version 1.7.1; DLL not rebuilt (the cut).**
