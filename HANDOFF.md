...
**Unreleased on main after v1.7.1 (2026-09-23): the build no longer embeds the build machine's folders.** Every shipped DLL through 1.7.1 carried the build machine's absolute PDB path in its PE debug directory (`C:\Users\<name>\…\obj\Release\TheRavensCall.pdb` since 1.3.0, a `/home/<user>/…` path before that). `TheRavensCall.csproj` now sets `DeterministicSourcePaths` (plus a `SourceRoot` of the project folder when there is no `.git`), so the DLL carries `/_/obj/Release/TheRavensCall.pdb` (`/_1/…` when built from a copy without `.git`, where the NuGet root takes `/_/`) and neither the DLL nor the PDB names a local path. Two clean builds of the same commit are byte-identical in any git clone of the repository, wherever it sits, with the same .NET SDK (proven: this checkout twice, a worktree at another path, and two reviewer clones, one path with a space). A copy without `.git` (e.g. GitHub's source zip) is path-free too but has no Source Link table and a different md5; a clone with a different origin URL also differs, because the PDB's Source Link URL carries it. The IL is unchanged (identical to 1.7.1 apart from static-data offsets, which move because the debug directory shrank). **At the next cut:** in the changelog's Changed section, say the DLL no longer carries "an absolute build path that included the build machine's user name" (quote no path); word the reproducibility line as "in any git clone of the repository with the same .NET SDK", no longer "in the same checkout"; and name the commit the DLL was built from ("a clean build of commit abcdef1"), because the md5 follows the commit: the PDB's Source Link URL carries the sha, so a rebuild of the tag commit itself differs in the 72 build-identity bytes. Porting this to a mod whose csproj sits in a subfolder: test the repo root's `.git` (e.g. `$(MSBuildProjectDirectory)\..\.git`), not the project folder's.
