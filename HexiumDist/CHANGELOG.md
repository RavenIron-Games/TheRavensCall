...
- **Live re-renders no longer thrash.** The "skip if the payload didn't change" check used to compare the raw `/api/state` text including `generated_at`, which changes every ten-second tick, so it never actually skipped anything against a live server. It now compares with `generated_at` stripped, and — since some re-renders are still real — any open `<details>` disclosure is remembered and reopened across one either way, instead of resetting collapsed every tick.
- **Ashlands can now light up.** The biome pip list spelled the key `AshLands`; `Saga.cs`'s `BiomeAndGearTracking.NormalizeBiome` has always emitted `Ashlands`, so the pip could never match and never lit, live server or not.
- **Caught fish resolve to real item names.** `caught_fish` values are lowercased Valheim localisation tokens (e.g. `$item_fish1`); the page now looks each one up against `/api/gamedata`'s `items[]` (case-insensitively, by `slug`) and shows the display name, falling back to a prettified token when gamedata isn't available.
- **The Death tab's server-narrated-deaths tile is gone.** `deaths_narrative` and `deaths_lifetime` are incremented on the same lines in `Saga.cs`, so they're always equal, and `deaths_narrative` has nothing to do with Discord/Chronicle narration despite its name. The tile is relabeled "Server-observed deaths" and reads `deaths_lifetime`.
- **Small correctness fixes:** `skill_progress` above 1 is now read as a percent instead of clamped to a full bar (vanilla sends 0–1; the server only clamps to 0–100); numeric-string counter keys the server's enum can't name (e.g. `"3968"`) are hidden instead of rendered; `SeekerBrute` and `SeekerSoldier` get their own creature labels instead of sharing "Seeker Soldier"; the gear tier card now shows the stored `gear_tier` value next to its "not reported on a dedicated server" note instead of just the note alone.
