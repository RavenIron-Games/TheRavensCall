...
### Added
- **The player's `/title` command**, with `WhereTheCrowFlies` 1.2.0+: `/title` (or the F5 console's `title`) lists what you've earned and which one is active; `/title <name>` (multi-word titles work, e.g. `/title Wolf Hunter`) sets it to one you've earned; `/title clear` goes back to showing no title. Travels over a new routed RPC pair — `RavensCall_EventReport_V2` event type 13 (`TitleRequest`) carrying the request, a new `RavensCall_TitleReply_V1` carrying the one-line answer back to that player only, never broadcast.
- **The admin's `ravenscall title <player> [<title>|clear]`** console subcommand — no client mod required, works from the server console or any admin's game client. Same earned-titles-only rule as the player path; the target player need not be online.
...
### Compatibility
- A `WhereTheCrowFlies` 1.2.0+ client against an older `TheRavensCall` server: `/title` sends its request and gets nothing back — the older server doesn't know event type 13, so the routed RPC is either dropped as "unknown eventType" (with `LogCombatReports` on) or, on a pre-1.7.0 server that predates type 13 entirely, ignored the same silent way any unregistered routed RPC always is. Either way the client prints a "no answer" hint after 5 seconds.
