...
| `events[].online_count` | int | Players in the world when the line was written: the same connected players `/api/state` counts, with sessions still open. Since 1.7.1 a `player_leave` row no longer counts the player who left, a peer still at the password prompt is never counted, and a name counts once. A player whose connection died without a disconnect still counts until ZRpc's ping timeout drops the peer, as in `/api/state`: 30 s, or 90 s for every peer on a crossplay server: every connection there, Steam players' included, comes in through `ZPlayFabSocket`, whose `Accept` sets ZRpc's static timeout to 90 s before the peer exists (only `ZNet.Start` sets it back to 30 s). |
