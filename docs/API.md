...
| `builders[]` | One row per distinct non-zero creator seen on any counted object, named or not; `name` is `null` when the server doesn't yet know that creator's name (a `builder_id` with no matching entry in `player_ids.json`). Each of `portals`, `beds`, `wards`, `ships`, `carts`, `chests`, `stations` on a row is that group's count for this builder; `pieces` is every object of *any kind* this builder placed and still stands in the world — not only the seven groups, so it is normally far larger than the group counts on the same row. |
